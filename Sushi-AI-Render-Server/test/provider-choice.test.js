...
test('manual image choice survives both successful and failed generation',async()=>{
 for(const engine of ['perchance','horde-real','horde-anime','sana']) for(const fail of [false,true]){
  ...
  const ctx={HORDE_BUDGET_MS:30000,engineFamily:e=>e==='horde-anime'||e==='sana'?'anime':'real',forcePhotorealPrompt:p=>p,applyEastAsianEthnicity:(p)=>p,withAdultDirective:p=>p,hasSmartModifier:()=>false,minimalOutboundPrompt:p=>p,ensureNoTextOnImage:p=>p,applyLocalEditOutbound:p=>p,value:()=>'',
...
test('unsupported reference image remains on selected provider',async()=>{
 const ctx={hasSmartModifier:()=>false,minimalOutboundPrompt:p=>p,ensureNoTextOnImage:p=>p,forcePhotorealPrompt:p=>p,applyEastAsianEthnicity:p=>p,withAdultDirective:p=>p,applyLocalEditOutbound:p=>p,engineFamily:()=>'real',value:()=>''};vm.createContext(ctx);vm.runInContext(extract(generation,'generateOne'),ctx);
